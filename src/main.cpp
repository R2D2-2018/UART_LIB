...
int main() {
    ...
    while (true) {
        ...
        ///< Receive something using the real UART hardware. You better use a buffer it you expect loads of data (the cout delays the receive process).
        ...
        if (availableFakeUART > 9) {
            ///< If you like, you can comment out the following code statement. This will demonstrate that the receive buffer of the fake UART is filled constantly
            ///< with the value of 0xAA
            ///< hwlib::cout << "Received from fake/testing UART: " << uartMockUser.receiveSomething() << hwlib::endl;
        }
    }
}
